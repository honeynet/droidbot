    ...
    def check_connectivity(self):
        ...
        for adapter in self.adapters:
            ...
            if not adapter_enabled:
                self.logger.warning("[CONNECTION] {0} is not enabled.".format(adapter_name))
            else:
                if adapter.check_connectivity():
                    self.logger.info("[CONNECTION] {0} is enabled and connected.".format(% adapter_name))
                else:
                    self.logger.info("[CONNECTION] {0} is enabled but not connected.".format(adapter_name))
